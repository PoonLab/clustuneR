...
setwd("~/git/clustuneR")
fasta <- read.FASTA("data/na.fasta")
coldates <- as.Date(sapply(names(fasta), function(x) strsplit(x, "_")[[1]][2]))
max.year <- max(year(coldates))
old.seqs <- fasta[year(coldates) < max.year]
...
Even so, this is a time-consuming step - to speed things up, we've provided IQ-TREE output files at `data/na.nwk` and `data/nwk.log`.
