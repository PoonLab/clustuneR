...
test_that("Tree can be extended with no new seqs", {
  expect_warning(expect_error(extend.tree(t=tree.full, seq.info=seq.info, mc.cores=1), NA),
...
test_that("Tree can be extended with new seqs to represent growth", {
  tree.old.sample <- drop.tip(tree.old, 1:799)
...
test_that("Monophyletic patristic clusters can be defined at boundary thresholds with no growth", {
  ...
  expect_warning(expect_error(multi.cluster(mono.pat.cluster, param.list, mc.cores = 1), NA),
                 "Method unfinished. Growth information for clusters not included")
})
